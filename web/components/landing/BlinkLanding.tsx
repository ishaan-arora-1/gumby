...
// Real template videos used by the Blink UGC app. Cloudinary was suspended
// (delivery 401s), so these are now re-hosted on Supabase Storage — the same
// approach as the one template that never broke (#0). The signed URLs live in
// landingVideos.ts, regenerated by backend/scripts/relink-cloudinary.js.
// Hardcoded here so the landing renders instantly even before the API responds.
const V = LANDING_VIDEOS;

// The hero wall — one entry per distinct creator so the grid feels varied.
// `fitness` reuses the existing Supabase gym clip (its source was never
// recovered from the dead Cloudinary account).
const TILE_VIDEOS = [
  { ...V.wall.fashion, label: 'Fashion' },
  { ...V.wall.skincare, label: 'Skincare' },
  { ...V.wall.beauty, label: 'Beauty' },
  { ...V.wall.jewellery, label: 'Jewellery' },
  { ...V.wall.evening, label: 'Evening' },
  { ...V.wall.fitness, label: 'Fitness' },
];
// Cards reuse the same Supabase clips (no per-size CDN transform anymore —
// the relink script already emits web-optimised 540p variants).
const CARD_VIDEOS = TILE_VIDEOS;

// Hand-picked "best of" videos featured in the Showcase section under
// the "One amazing video is all it takes to sell." headline. The middle
// tile is still served locally from /public/showcase/.
const SHOWCASE_VIDEOS = [
  // Blue-block caption variant — generated by our pipeline
  V.showcase.demo,
  // Middle slot — local pink-block captioned jewellery clip.
  {
...
function VideoMockup() {
  // left, center, right — Supabase-hosted (Cloudinary suspended). Center used
  // to be the captioned-skincare clip; its source wasn't recovered, so it now
  // plays the clean skincare clip. Right reuses the existing Supabase gym clip.
  const c = [
    LANDING_VIDEOS.mockup.evening,
    LANDING_VIDEOS.mockup.skincare,
    LANDING_VIDEOS.mockup.fitness,
  ];
  const layout = [
